...
abstract class BaseRepository implements RepositoryCriteriaInterface, RepositoryInterface
{
     ...
     * @return Collection|Model
     */
    public function getByIdOrSlug($id, array $columns = ['*'])
    {
        $this->unsetClauses();
        ...
        return $this->query->where(
            function ($query) use ($id) {
                $query->orWhere('id', $id)->orWhere('slug', $id);
            }
        )->firstOrFail($columns);
    }
    ...
    public function paginate($limit = 15, array $columns = ['*'], $pageName = 'page', $page = null)
    {
        $this->applyCriteria();
        $this->newQuery()->eagerLoad()->setClauses()->setScopes();
        $models = $this->query->paginate(request()->per_page ?? $limit, $columns, $pageName, $page);
